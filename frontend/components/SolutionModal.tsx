...
// Cache solutions per problem+lang to avoid duplicate API calls
const solutionCache = new Map<string, Approach[]>();
...
export default function SolutionModal() {
  ...
  const handleConfirm = async () => {
    ...
    // Check cache first
    const cacheKey = `${problem?.titleSlug}:${currentLang}`;
    const cached = solutionCache.get(cacheKey);
    if (cached) {
      setApproaches(cached);
      return;
    }
    ...
    try {
      ...
      if (data.approaches) {
        ...
        // Cache for future re-opens
        solutionCache.set(cacheKey, data.approaches);
      } else {
        const errorMsg = data.retryAfter
          ? `Rate limit reached. Please retry in ${data.retryAfter}s.`
          : (data.error ?? "Failed to load solution");
  ...
  return (
    <AnimatePresence>
      {showSolutionModal && problem && <motion.div
        key="overlay"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
