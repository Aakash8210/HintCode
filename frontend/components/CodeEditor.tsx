...
export default function CodeEditor() {
  ...
  const handleSubmitCode = useCallback(async () => {
    ...
    try {
      ...
      if (data.submission) {
        ...
        if (accepted) {
          toast.success("Submitted and accepted");
        } else {
          toast.error("Submitted with failing tests");
        }
      }
    } catch {
      toast.error("Submit failed. Check your connection.");
    } finally {
      setIsSubmitting(false);
    }
  }, [problem, userCode, currentLang, setTestResults]);
  ...
  return (
    <div className="h-full flex flex-col bg-[#1a1a1a]">
      ...
      <div className="flex items-center justify-between px-3 py-2 border-b border-[#2a2a2a] flex-shrink-0 gap-3">
        {/* Language tabs */}
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            disabled={isRunningCode || isSubmitting || !problem}
          ...
          <button
            ...
            disabled={isRunningCode || isSubmitting || !problem}
            className="text-xs px-4 py-1.5 rounded-lg border border-amber-500/40 text-amber-400 hover:bg-amber-500/10 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-1.5"
          >
            {isSubmitting ? (
              <>
      ...
      {/* Test Results */}
      {(testResults || isRunningCode || isSubmitting) && (
        <motion.div
          initial={{ height: 0, opacity: 0 }}
          animate={{ height: "auto", opacity: 1 }}
          className="border-t border-[#2a2a2a] max-h-56 overflow-y-auto flex-shrink-0"
        >
          <TestResultsTable results={testResults} loading={isRunningCode || isSubmitting} />
        </motion.div>
      )}
      <SubmissionHistory refreshKey={submissionRefreshKey} />
    </div>
  );
}
