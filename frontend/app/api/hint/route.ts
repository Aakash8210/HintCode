...
const LOCAL_FALLBACK_HINTS = [
  "What exactly must be returned for one valid example, and what condition confirms a pair/answer is valid?",
  "If you needed constant-time lookup while scanning once, what information would you store as you go?",
  "While iterating, can you derive what value would complete the answer and check if it was seen earlier?",
  "Try this structure: initialize memory map → loop input → compute needed value → check map → otherwise store current value.",
  "Sketch partial logic with blanks:\n1) create lookup ___\n2) for each item ___\n3) needed = target - current\n4) if needed in lookup: return ___\n5) else store ___",
];

export async function POST(req: NextRequest) {
  let safeHintNumber = 1;
  try {
    ...
    safeHintNumber = Math.min(Math.max(1, hint_number), 5);
...
Generate hint ${hint_number} following your system instructions exactly. Be concise and Socratic.`;
    ...
    if (status === 429 || status === 503) {
      const localHint = LOCAL_FALLBACK_HINTS[safeHintNumber - 1];
      return NextResponse.json(
        {
          hint: localHint,
          fallback: true,
          reason: "AI hint service is temporarily unavailable",
        },
        { status: 200 }
      );
    }
